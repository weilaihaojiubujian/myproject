<template>
    <div style="height:500px;overflow:scroll;">
        ...
        <ul  v-for="(problem,index) in tdata2" :key="index">
            <div   class="myUserAvatar" style="line-height:70px;display:inline-block;margin-top: 5px;">
                <img style="width:45px;height:45px;border-radius: 50%;float: left;background-size: 45px 45px;" @click="otherUserInfoByUserId(problem.userId)" :src=" problem.portraitUrl || this.imgSrc" />
...
<!--        <Page :total="dataCount" :page-size="pageSize" show-total @on-change="changepage"></Page>-->
        ...
        <el-dialog title="新增" v-model="addFormVisible" :visible.sync="addFormVisible">
            <el-form :model="problem" label-width="80px"  ref="problem">
                <el-form-item label="问题内容" prop="content">
                    <el-input  type="textarea" v-model="problem.content" auto-complete="off"></el-input>
                </el-form-item>
            </el-form>
...
//                 ],
        ...
        methods: {
            ...
            changepage(index) {
                ...
                this.problemListRequest.pageNo=index;
                this.$axios.post(api.problemList, JSON.stringify(this.problemListRequest), {
                    headers: {
                        'Access-Control-Allow-Origin': '*',
                        'Content-Type': 'application/json; charset=utf-8'
            ...
            handleComment(id,content) {
                ...
                this.$axios.post(api.addClick, JSON.stringify(this.addClickRequest), {
                    headers: {
                        'Access-Control-Allow-Origin': '*',
                        'Content-Type': 'application/json; charset=utf-8'
                    },
                    ...
                    if (res != null && res.status === 200) {
                        if (res.data.success) {
                            console.log(id);
                            this.$router.push({ name:'评论列表', params:{id:id,content:content}});
                        } else {
                            console.log(res);
                        }
                    } else {
                ...
                this.$refs.problem.validate((valid) => {
                    if (valid) {
                        this.$confirm('确认提交吗？', '提示', {}).then(() => {
                            ...
                            this.$axios.post(api.createProblem, JSON.stringify(this.problem), {
                                headers: {
                                    'Access-Control-Allow-Origin': '*',
                                    'Content-Type': 'application/json; charset=utf-8'
                                },
